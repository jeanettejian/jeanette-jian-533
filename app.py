...
ek.set_app_key(os.getenv('APP_KEY'))
...
@app.callback(
    Output("history-tbl", "data"), # updating data
    Input("run-query", "n_clicks"),
    [State('benchmark-id', 'value'), State('asset-id', 'value')],
    prevent_initial_call=True # don't want to run immediately, want to run after clicking the button (on the dash)
)
...
def calculate_returns(history_tbl):
    ...
    dt_prc_div_splt[dte_col] = pd.to_datetime(dt_prc_div_splt[dte_col]) # date object
